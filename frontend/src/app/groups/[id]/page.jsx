...
export default function GroupDetailPage() {
    ...
    if (cargando) return <div className="bg-gray-900 min-h-screen text-white flex items-center justify-center font-mono text-xs">Loading...</div>;
    if (!grupo) return <div className="bg-gray-900 min-h-screen text-white flex items-center justify-center">Group not found.</div>;
    ...
    return (
        <div className="bg-gray-900 min-h-screen pb-10 text-white relative">
            ...
            <div className="max-w-6xl mx-auto pt-24 px-6 md:pl-64">

                <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4 border-b border-gray-800 pb-6">
                    <div>
                        <span className="text-xs text-gray-500 uppercase tracking-widest font-mono flex items-center gap-1 mb-1">🏠 Group</span>
                        <h1 className="text-3xl font-bold text-gray-100 mb-2">{grupo.name}</h1>
                        <p className="text-sm text-gray-400 max-w-xl">{grupo.description || "No description provided."}</p>
                    </div>
                    <div className="flex gap-3 self-end md:self-auto">
                        ...
                        <button className="bg-yellow-400 hover:bg-yellow-500 text-black font-bold px-4 py-2 rounded-xl text-sm">
                            ➕ Add Expense
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    ...
                    <div className="md:col-span-7 space-y-4">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button className="text-xs text-yellow-400/80 hover:text-yellow-400 font-mono">Filter 🔽</button>
                        </div>

                        {gastos.length === 0 ? (
                            <p className="text-gray-500 text-xs italic font-mono p-4 bg-gray-800/20 rounded-xl border border-gray-800">
                                No expenses registered in this group yet.
                            </p>
                        ) : (
                            gastos.map((gasto) => (
                                <div key={gasto.id} className="bg-gray-800/60 rounded-xl p-4 border border-gray-700/50 flex justify-between items-center border-l-4 border-l-green-500">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-gray-900 rounded-xl flex items-center justify-center text-lg border border-gray-700">
                                            💸
                                        </div>
                                        <div>
                                            <h4 className="text-sm font-bold text-gray-200">{gasto.description}</h4>
                                            <p className="text-xs text-gray-500">Paid by User ID: {gasto.paid_by}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="text-right flex gap-6">
                                        <div className="min-w-[60px]">
                                            ...
                                            <p className="text-sm font-semibold text-gray-400">€{gasto.amount.toFixed(2)}</p>
                                        </div>
                                    </div>
                                </div>
                            ))
                    ...
                    <div className="md:col-span-5 space-y-6">
                        <div className="bg-gray-800/90 rounded-2xl p-5 border border-gray-700/70 space-y-4">
                            <div>
                                <h3 className="text-base font-bold text-gray-200">Balances & Members</h3>
                                <p className="text-xs text-gray-500">Active users assigned to this group via backend.</p>
                            </div>
                                ...
                                {miembros.map((miembro, idx) => (
                                    <div key={idx} className="flex justify-between items-center text-sm bg-gray-900/40 p-2.5 rounded-xl border border-gray-800/40">
                                        <div className="flex items-center gap-2">
                                            <div className="w-7 h-7 bg-gray-800 rounded-full flex items-center justify-center text-xs border border-gray-700 text-gray-400">👤</div>
                        ...
                        <div className="bg-gray-800/40 rounded-2xl p-4 border border-gray-800 space-y-3 text-xs">
                            <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                                <span className="font-semibold text-gray-400">Group Info</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-500">Total Group Cost</span>
                                <span className="font-mono text-gray-300">€{totalExpensesSum.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between">
